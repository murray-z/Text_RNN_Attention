...
from text_rnn_attention import TextRnnAttention
...
def train(config):
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session(config=sess_config) as sess:
            rnn_att = TextRnnAttention(config)

        # training procedure
        global_step = tf.Variable(0, name='global_step', trainable=False)
        optimizer = tf.train.AdamOptimizer(config['learning_rate'])
        grads_and_vars = optimizer.compute_gradients(rnn_att.loss)
        ...
        loss_summary = tf.summary.scalar('loss', rnn_att.loss)
        acc_summary = tf.summary.scalar('accuracy', rnn_att.accuracy)
        ...
        def train_step(x_batch, y_batch):
            feed_dict = {
                rnn_att.input_x: x_batch,
                rnn_att.input_y: y_batch,
                rnn_att.dropout_keep_prob: config['dropout_keep_prob']
            }

            _, step, summaries, loss, accuracy = sess.run(
                [train_op, global_step, train_summary_op, rnn_att.loss, rnn_att.accuracy],
                feed_dict=feed_dict
            )
        ...
        def dev_step(x_batch, y_batch, writer=None):
            feed_dic = {
                rnn_att.input_x: x_batch,
                rnn_att.input_y: y_batch,
                rnn_att.dropout_keep_prob: 1.0
            }

            step, summaries, loss, accuracy = sess.run(
                [global_step, dev_summary_op, rnn_att.loss, rnn_att.accuracy],
                feed_dict=feed_dic
            )
        ...
        # test accuracy
        test_accuracy = sess.run([rnn_att.accuracy], feed_dict={
            rnn_att.input_x: x_test, rnn_att.input_y: y_test, rnn_att.dropout_keep_prob: 1.0})
